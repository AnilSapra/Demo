...
#SIDEBAR
with st.sidebar:
    selected=option_menu("Main Menu",["Dataset","Overview","Ride Analytics","Data Assistant"],icons=["table","bar-chart","graph-up"],menu_icon="car-front",default_index=3)
...
if selected=="Ride Analytics":
    ...
    fig = px.bar(demand, x="Vehicle Type", y="Total Bookings", color="Vehicle Type")
    st.plotly_chart(fig, use_container_width=True)
    st.divider()

    #Revenue by Vehicle Type
    st.subheader("Revenue by Vehicle Type")
    revenue=df.groupby("Vehicle Type")["Booking Value"].sum().reset_index()
    fig = px.bar(revenue, y="Vehicle Type", x="Booking Value", orientation='h', color="Vehicle Type")
    st.plotly_chart(fig, use_container_width=True)
    st.divider()
    ...
    fig = px.pie(payment, names="Method", values="Count")
    st.plotly_chart(fig, use_container_width=True)
    st.divider()

    #Ride Distance vs Booking Value
    st.subheader("Ride Distance vs Booking Value")
    fig=px.scatter(df,x=df["Ride Distance"],y=df["Booking Value"],color="Vehicle Type")
    st.plotly_chart(fig,use_container_width=True)
    st.divider()

    #Customer Rating Distribution
    st.subheader("Customer Rating Distribution")
    fig=px.histogram(df,x="Customer Rating",nbins=20)
    st.plotly_chart(fig,use_container_width=True)
    ...
    fig=px.bar(cancel_reason.head(10),y="Cancellation Reason",x="Count",orientation="h")
    st.plotly_chart(fig,use_container_width=True)
    st.divider()
    ...
    fig=px.bar(avg_dist,x="Vehicle Type",y="Ride Distance",color="Vehicle Type")
    st.plotly_chart(fig,use_container_width=True)
    st.divider()

    #Booking Value Distribution
    st.subheader("Booking Value Distribution")
    fig=px.histogram(df,x="Booking Value",nbins=30)
    st.plotly_chart(fig,use_container_width=True)
    st.divider()

    # Operational Efficiency (CTAT vs VTAT)
    st.subheader(" Operational Efficiency (CTAT vs VTAT)")
    fig=px.scatter(df,x="Avg CTAT",y="Avg VTAT",color="Vehicle Type")
    st.plotly_chart(fig,use_container_width=True)
...
if selected=="Data Assistant":
    ...
    st.write("Ask Questions About The Dataset and Get Visual Anlytics")
